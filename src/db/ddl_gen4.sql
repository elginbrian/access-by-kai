...
-- ========================================
-- BAGIAN XV: SISTEM REVIEW SEDERHANA
-- ========================================

-- Tipe Layanan untuk Review
CREATE TYPE jenis_layanan_enum AS ENUM ('BOOKING_TIKET', 'EPORTER', 'LOGISTIK', 'LAPORAN_MANUAL');

-- Tabel Review Sederhana
CREATE TABLE ulasan (
    ulasan_id BIGSERIAL PRIMARY KEY,
    pengguna_id BIGINT NOT NULL REFERENCES pengguna(user_id),
    jenis_layanan jenis_layanan_enum NOT NULL,
    penilaian SMALLINT NOT NULL CHECK (penilaian BETWEEN 1 AND 5),
    komentar TEXT NOT NULL,
    platform VARCHAR(20) DEFAULT 'UMUM',
    dibuat_pada TIMESTAMPTZ DEFAULT now(),
    diperbarui_pada TIMESTAMPTZ DEFAULT now()
);

-- Index untuk Performance
CREATE INDEX idx_ulasan_pengguna ON ulasan (pengguna_id);
CREATE INDEX idx_ulasan_layanan ON ulasan (jenis_layanan);
CREATE INDEX idx_ulasan_penilaian ON ulasan (penilaian);
CREATE INDEX idx_ulasan_dibuat ON ulasan (dibuat_pada);

-- Dokumentasi
COMMENT ON TABLE ulasan IS 'Sistem ulasan sederhana untuk 4 layanan utama: Booking Tiket, E-Porter, Logistik, Laporan Manual';
COMMENT ON COLUMN ulasan.jenis_layanan IS 'Jenis layanan: BOOKING_TIKET, EPORTER, LOGISTIK, LAPORAN_MANUAL';
COMMENT ON COLUMN ulasan.komentar IS 'Komentar dan ulasan pengguna tentang layanan';