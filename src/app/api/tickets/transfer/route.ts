...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!ticketId || !targetNik || !targetNama) {
      return NextResponse.json({ error: "Ticket ID, NIK, dan nama target harus diisi" }, { status: 400 });
    }
    ...
    const { data: allUsers, error: allUsersError } = await supabase.from("pengguna").select("user_id, nama_lengkap, nik, nomor_identitas, email").limit(10);
    ...
    const { data: userWithIdentitas, error: identitasSearchError } = await supabase.from("pengguna").select("user_id, nama_lengkap, nik, nomor_identitas").eq("nomor_identitas", targetNik).eq("nama_lengkap", targetNama).single();

    if (userWithIdentitas && !identitasSearchError) {
      targetUser = userWithIdentitas;
    } else {
      // Fallback: coba cari hanya dengan nama
      const { data: userWithName, error: nameSearchError } = await supabase.from("pengguna").select("user_id, nama_lengkap, nik, nomor_identitas").eq("nama_lengkap", targetNama).single();
    ...
    if (!targetUser) {
      ...
      return NextResponse.json({ error: "Pengguna dengan NIK dan nama tersebut tidak ditemukan" }, { status: 404 });
    }

    // 2. Akses data tiket untuk mendapatkan penumpang_id
    const { data: ticketData, error: ticketError } = await supabase.from("tiket").select("tiket_id, penumpang_id").eq("tiket_id", ticketId).single();
    ...
    if (penumpangError || !penumpangData) {
      ...
      return NextResponse.json({ error: "Data penumpang tidak ditemukan" }, { status: 404 });
    }
    ...
    if (currentUserError || !currentUserProfile || currentUserProfile.user_id !== currentOwnerId) {
      return NextResponse.json({ error: "Anda tidak memiliki izin untuk mentransfer tiket ini" }, { status: 403 });
    }

    // 4. Check if target user is different from current owner
    if (targetUser.user_id === currentOwnerId) {
      return NextResponse.json({ error: "Tidak dapat mentransfer tiket ke diri sendiri" }, { status: 400 });
    }
    ...
    if (createPenumpangError || !newPenumpang) {
      ...
      return NextResponse.json({ error: "Gagal membuat data penumpang baru" }, { status: 500 });
    }

    // 5. Update tiket untuk mengarah ke penumpang baru (hanya tiket yang ditransfer)
    const { error: transferError } = await supabase.from("tiket").update({ penumpang_id: newPenumpang.penumpang_id }).eq("tiket_id", ticketId);

    if (transferError) {
      console.error("Transfer error:", transferError);
      return NextResponse.json({ error: "Gagal mentransfer tiket. Silakan coba lagi." }, { status: 500 });
    }
