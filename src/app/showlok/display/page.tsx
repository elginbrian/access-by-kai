...
const ShowlokDisplayPage: React.FC = () => {
  ...
  const { data: stationId } = useQuery({
    ...
    queryFn: async (): Promise<number | null> => {
      ...
      const stationInfo = selectedStation === 'departure'
        ? selectedTicket.departureStation
        : selectedTicket.arrivalStation;
      ...
      <div className="container mx-auto px-8 py-16">
        ...
        <div className="mb-8">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-lg mb-6">
            ...
            <div className="text-sm text-gray-600">
              <p><strong>{selectedTicket.trainName || selectedTicket.ticketNumber}</strong></p>
              <p>{selectedTicket.departureStation?.name} → {selectedTicket.arrivalStation?.name}</p>
        ...
        <div className="mt-8">
          <h2 className="text-xl font-semibold text-gray-900 mb-6">
            Fasilitas di {selectedStation === 'departure' ? selectedTicket.departureStation?.name : selectedTicket.arrivalStation?.name}
          </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {facilities.map((facility) => (
                <FacilityCard
                  ...
                  facility={facility}
                  onSelect={() => handleFacilitySelect(facility.facility_id)}
                />
              ))}
            </div>
          )}
