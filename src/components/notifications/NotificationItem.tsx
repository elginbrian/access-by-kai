...
const NotificationItem: React.FC<NotificationItemProps> = ({
  ...
  const handleMarkAsRead = () => {
    if (!notification.is_read) {
      markAsReadMutation.mutate(
        { id: notification.notification_id, is_read: true },
        {
          onSuccess: () => {
            onRead?.(notification);
          },
  ...
  return (
    <div
      className={`p-4 border-l-4 rounded-lg mb-3 transition-all duration-200 hover:shadow-md ${
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-start space-x-3 flex-1">
          <div className="text-2xl mt-1">
            {getTypeIcon(notification.tipe_notifikasi)}
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between text-xs text-gray-400">
              <span>{timeAgo}</span>
              <span className="capitalize bg-gray-100 px-2 py-1 rounded text-gray-600">
                {notification.tipe_notifikasi.replace('_', ' ').toLowerCase()}
              </span>
            </div>
          </div>
        </div>
