'use client';

import React from 'react';

interface Passenger {
    id: string;
    name: string;
    type: 'Adult' | 'Child';
    isSelected: boolean;
}

interface PassengerListCardProps {
    passengers: Passenger[];
    maxPorters: number;
}

const PassengerListCard: React.FC<PassengerListCardProps> = ({
    passengers,
    maxPorters
}) => {
    return (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center gap-3 mb-6">
                <div className="bg-blue-100 p-3 rounded-xl">
                    <img src="/ic_family_seat_blue.svg" alt="Passengers" className="w-5 h-5" />
                </div>
                <div>
                    <h3 className="text-lg font-semibold text-gray-900">Select Passengers</h3>
                    <p className="text-sm text-gray-500">Choose which passengers need porter assistance</p>
                </div>
            </div>

            <div className="space-y-4 mb-6">
                {passengers.map((passenger) => (
                    <div key={passenger.id} className="flex items-center justify-between bg-gray-50 p-2 rounded-lg">
                        <div className="flex items-center gap-3">
                            <div>
                                <span className="text-gray-900 font-medium">
                                    {passenger.name}
                                </span>
                                <p className="text-sm text-gray-500">{passenger.type}</p>
                            </div>
                        </div>
                        <span className={`px-3 py-1 rounded-full text-sm font-medium ${passenger.type === 'Adult'
                            ? 'bg-blue-100 text-blue-700'
                            : 'bg-green-100 text-green-700'
                            }`}>
                            {passenger.type}
                        </span>
                    </div>
                ))}
            </div>

            <div className="bg-blue-50 rounded-lg p-4 flex items-start gap-3">
                <svg className="w-5 h-5 text-blue-600 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
                </svg>
                <div>
                    <p className="text-sm text-blue-800">
                        <strong>Maximum porters allowed: {maxPorters}</strong> (equal to total passengers booked)
                    </p>
                </div>
            </div>
        </div>
    );
};

export default PassengerListCard;