...
const OrderSummary: React.FC<OrderSummaryProps> = ({ cart, onRemoveFromCart, formatPrice, subtotal, serviceCharge, total, onCheckout }) => {
  return (
    <div className="bg-white rounded-2xl border p-6 sticky top-6">
        ...
        {cart.map((item) => (
          <div key={item.id} className="flex justify-between items-start">
            ...
            <div className="flex items-center gap-2 ml-4">
              ...
              <button onClick={() => onRemoveFromCart(item.id)} className="text-orange-400 hover:bg-yellow-200 p-1 rounded-lg">
      ...
      {/* Checkout Button */}
      <button onClick={onCheckout} className="w-full bg-blue-600 text-white py-4 rounded-xl font-semibold hover:bg-blue-700 transition-colors mb-4 bg-gradient-to-b from-[#6b46c1] to-[#3b82f6]">
        Lanjut ke Review
      </button>

      <p className="text-xs text-black text-center">Makanan akan disiapkan sebelum keberangkatan</p>
    </div>
  );
};
