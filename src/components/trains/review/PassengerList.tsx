  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-xl font-bold text-black">Penumpang & Kursi</h2>
      </div>
          ...
          <div key={passenger.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12">
                <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(passenger.name)}`} alt={`${passenger.name} avatar`} className="w-12 h-12 rounded-full object-cover border-2 border-gray-200 bg-white" />
              </div>

              <div>
                <h3 className="font-semibold text-black">{passenger.name}</h3>
                <p className="text-sm text-black">Dewasa • ID: {passenger.idNumber}</p>
              </div>
            </div>
