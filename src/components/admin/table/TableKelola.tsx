...
export default function TableKelola<T extends { id?: string }>(props: Props<T>) {
  const { title, description, data = defaultMock, columns, perPage = 10, onEdit, onDelete, addButtonLabel = "Tambah", onAdd } = props;
    ...
    <div className="bg-white rounded-lg shadow-sm overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-3">
          ...
          <div className="flex gap-2">
            <select
              ...
              className="h-11 rounded-lg border px-4 text-sm text-gray-600"
            >
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-left">
          <thead className="text-sm text-gray-500">
            <tr>
              {columns.map((col) => (
                <th key={col.key} className={`py-5 px-6 ${col.className ?? ""}`}>
                  {col.label ?? col.key}
                </th>
              ))}
                    ...
                    <div className="flex items-center gap-3 justify-end">
                      {onEdit && (
                        <button onClick={() => onEdit(row)} title="Edit" className="p-2 rounded-md hover:bg-gray-100">
                          <Icon name="edit" className="w-5 h-5 text-gray-500" />
                        </button>
                      )}
                      {onDelete && (
                        <button onClick={() => onDelete(row)} title="Hapus" className="p-2 rounded-md hover:bg-gray-100">
                          <Icon name="trash" className="w-5 h-5 text-gray-400" />
                        </button>
                      )}
                    </div>
                  </td>
      ...
      <div className="px-6 py-4 border-t flex items-center justify-between">
        <div className="text-sm text-gray-500">
          Menampilkan {start + 1}-{Math.min(start + perPage, total)} dari {total} pengiriman
        </div>

        <div className="flex items-center gap-2">
          <button onClick={() => setPage((p) => Math.max(1, p - 1))} className="px-3 py-2 rounded-md border text-sm text-gray-600">
            Previous
          </button>
          <div className="flex items-center gap-2">
            {Array.from({ length: pages }).map((_, i) => (
              <button key={i} onClick={() => setPage(i + 1)} className={`px-3 py-2 rounded-md border text-sm ${page === i + 1 ? "bg-blue-600 text-white" : "text-gray-600 bg-white"}`}>
                {i + 1}
              </button>
            ))}
          </div>
          <button onClick={() => setPage((p) => Math.min(pages, p + 1))} className="px-3 py-2 rounded-md border text-sm text-gray-600">
            Next
          </button>
        </div>
      </div>
