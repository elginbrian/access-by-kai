...
// Query Keys
const QUERY_KEYS = {
  ulasan: ["ulasan"] as const,
  ulasanById: (id: number) => ["ulasan", id] as const,
  ulasanByUser: (userId: number) => ["ulasan", "user", userId] as const,
  ulasanByService: (service: string) => ["ulasan", "service", service] as const,
};

// List ulasan with filters
export function useUlasanList(filters?: {
